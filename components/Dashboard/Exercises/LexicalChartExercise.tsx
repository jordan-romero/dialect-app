...
import { Box, Button, Text, VStack, HStack, Grid, GridItem, Flex, Badge, Tooltip, Tabs, TabList, TabPanels, Tab, TabPanel } from '@chakra-ui/react'
...
export const LexicalChartExercise: React.FC<LexicalChartExerciseProps> = ({
  ...
  const [currentCategory, setCurrentCategory] = useState<'monophthongs' | 'diphthongs' | 'triphthongs'>('monophthongs')

  // Load lexical chart data
  useEffect(() => {
    const loadChartData = async () => {
      try {
        console.log('Loading lexical chart data for lessonId:', lessonId, 'quizIndex:', quizIndex)
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.answers && data.answers.length > 0) {
            const savedAnswer = data.answers.find((answer: any) =>
              answer.questionId === chartData.questions[0]?.id
            )
            if (savedAnswer && savedAnswer.textAnswer && savedAnswer.textAnswer !== 'pending') {
              try {
  ...
  const checkCategoryCompletion = (category: string) => {
    ...
    const categoryItems = chartData.lexicalItems.filter(item => item.category === category)
    return categoryItems.every(item => userAnswers[item.id] === item.ipaSymbol)
  }

  const checkOverallCompletion = () => {
    if (!chartData) return false
    return chartData.lexicalItems.every(item => userAnswers[item.id] === item.ipaSymbol)
  }
  ...
  const getCategoryItems = () => {
    if (!chartData) return []
    return chartData.lexicalItems.filter(item => item.category === currentCategory)
  }
  ...
  return (
    <VStack spacing={4} align="stretch" maxW="1000px" mx="auto" p={4}>
      <Text fontSize="xl" fontWeight="bold" textAlign="center">
        {chartData.questions[0]?.text || 'Instructions: Click on IPA symbols from the bank below and place them in their correct corresponding spots for a General American dialect.'}
      </Text>

      {/* Instructions */}
      <Box bg="gray.50" p={3} borderRadius="lg" border="1px solid" borderColor="gray.200">
        <Text fontSize="sm" color="black">
          <Text as="span" fontWeight="bold">Instructions:</Text> Click on a symbol from the bank below, then click on the IPA Symbol position next to the corresponding example word. Hover over example words to see additional words with the same sound. Double-click on a placed symbol to remove it.
        </Text>
      </Box>
      ...
      >
        ...
        <Flex wrap="wrap" gap={2}>
          {getAvailableSymbols().map((symbol, index) => (
            <Button
              ...
              variant={selectedSymbol === symbol ? 'brandBold' : 'brandWhite'}
        ...
        {selectedSymbol && (
          <Text mt={3} fontSize="sm" fontWeight="medium">
            Selected: <Text as="span" fontFamily="'Charis SIL', serif" fontWeight="bold" fontSize="lg">{selectedSymbol}</Text> - Click on a position to place it
          </Text>
        )}
      </Box>
      ...
      <Box
        ...
        boxShadow="sm"
      >
        <Tabs index={currentCategory === 'monophthongs' ? 0 : currentCategory === 'diphthongs' ? 1 : 2} onChange={(index) => {
          const categories = ['monophthongs', 'diphthongs', 'triphthongs'] as const
          ...
          <TabList>
            <Tab>
              ...
              {checkCategoryCompletion('monophthongs') && (
                <Badge ml={2} colorScheme="green" borderRadius="full">✓</Badge>
              )}
            </Tab>
            <Tab>
              Diphthongs
          ...
          <TabPanels>
            {(['monophthongs', 'diphthongs', 'triphthongs'] as const).map(category => (
              <TabPanel key={category} p={4} minH="400px" maxH="500px" overflowY="auto">
                <Grid templateColumns="1fr 1fr" gap={3}>
                  <GridItem>
                    <Text fontWeight="bold" mb={2}>Example Word</Text>
                  </GridItem>
                  <GridItem>
                    <Text fontWeight="bold" mb={2}>IPA Symbol</Text>
                  </GridItem>
                        ...
                        <GridItem>
                          <Box
                            ...
                            borderColor={
                              isCorrect ? 'green.500' :
                              hasAnswer ? 'red.500' :
                              selectedSymbol ? 'brand.iris' : 'brand.blue'
                            }
                            borderRadius="md"
                            display="flex"
                            alignItems="center"
                          ...
                          >
                            {hasAnswer ? (
                              <Text fontSize="lg" fontWeight="bold" fontFamily="'Charis SIL', serif">
                                {userAnswer}
                              </Text>
                            ) : (
      ...
      {isCompleted && (
        <Box mt={2} p={3} bg="green.100" borderRadius="lg" border="1px solid" borderColor="green.300">
